...
{
//    std::cout << "RegisterTX: " << arbId << ", " << periodMs << ", " << dlc << ", " << (initialFrame == NULL) << std::endl;
	int32_t status = 0;
	if(dlc > 8)
		dlc = 8;
	txJob_t job;
...
//	if(timeoutMs > 999)
//		timeoutMs = 999;
	HAL_CAN_ReceiveMessage(&arbId,kFullMessageIDMask,dataBytes,&len,&timeStamp,&status);
//	if(status == 0){
//		/* fresh update */
...
{
//    std::cout << "FlushTx" << std::endl;
	int32_t status = 0;
	txJobs_t::iterator iter = _txJobs.find(arbId);
	if(iter != _txJobs.end())
	    HAL_CAN_SendMessage(	iter->second.arbId,
															iter->second.toSend,
															iter->second.dlc,
...
{
//    std::cout << "ChangeTX Period" << std::endl;
//	int32_t status = 0;
