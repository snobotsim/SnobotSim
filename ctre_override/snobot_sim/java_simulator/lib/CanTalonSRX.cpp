...
#define CAN_SIMULATION_UNSUPPORTED_LOG()                                               \
{                                                                                      \
    std::cout << "Unsupported function in CanTalonSRX: " << __FUNCTION_NAME__ << std::endl; \
}

#define CAN_SIMULATION_UNSUPPORTED() CAN_SIMULATION_UNSUPPORTED_LOG(); return CTR_InvalidParamValue;


CanTalonData SimCanTalonData[64];
...
{
    SimCanTalonData[mDeviceNumber].HandleInitialized();
}
/* CanTalonSRX D'tor
 */
...
void CanTalonSRX::Set(double value) {
    SimCanTalonData[mDeviceNumber].HandleSet(value);
}
...
CTR_Code CanTalonSRX::SetParam(param_t paramEnum, double value) {
    SimCanTalonData[mDeviceNumber].SetParam(paramEnum, value);
    return CTR_OKAY;
}
...
CTR_Code CanTalonSRX::SetModeSelect(int modeSelect, int demand) {
    SimCanTalonData[mDeviceNumber].SetModeSelect(modeSelect, demand);
    return CTR_OKAY;
}
/**
...
{
    param = SimCanTalonData[mDeviceNumber].GetAppliedThrottle();
    return CTR_OKAY;
}
...
{
    param = SimCanTalonData[mDeviceNumber].GetSensorPosition();
    return CTR_OKAY;
}
CTR_Code CanTalonSRX::GetSensorVelocity(int &param)
{
    param = SimCanTalonData[mDeviceNumber].GetSensorVelocity();
    return CTR_OKAY;
}
CTR_Code CanTalonSRX::GetCurrent(double &param)
{
    param = SimCanTalonData[mDeviceNumber].GetCurrent();
    return CTR_OKAY;
}
...
{
    param = SimCanTalonData[mDeviceNumber].GetEncPosition();
    return CTR_OKAY;
}
CTR_Code CanTalonSRX::GetEncVel(int &param)
{
    param = SimCanTalonData[mDeviceNumber].GetEncVel();
    return CTR_OKAY;
}
...
{
    SimCanTalonData[mDeviceNumber].SetDemand(param);
    return CTR_OKAY;
}
...
{
    SimCanTalonData[mDeviceNumber].SetFeedbackDeviceSelect(param);
    return CTR_OKAY;
}
...
{
    SimCanTalonData[mDeviceNumber].SetModeSelect(param);
    return CTR_OKAY;
}
