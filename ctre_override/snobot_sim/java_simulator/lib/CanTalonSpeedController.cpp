...
double CanTalonSpeedController::GetSensorVelocity()
{
	if(mFeedbackDevice != ConnectedSensor_Encoder)
	{
		SNOBOT_SIM_CAN_LOG_UNSUPPORTED_WITH_MESSAGE("Feedback device not supported");
	}
//	return GetVelocity();
    return 0;
}
...
//		return GetPosition();
	...
	SNOBOT_SIM_CAN_LOG("Feedback device is not an encoder");
	return 0;
}
...
//        SetVoltagePercentage(mLastSetValue);
    ...
    {
        SNOBOT_SIM_CAN_LOG("This shouldn't be called directly");
        break;
    }
    default:
        SNOBOT_SIM_CAN_LOG("Unknown control mode");
    }
}
