...
public class TestTankDriveSimulator extends BaseSimulatorTest
{
    ...
    // @Test
    // public void testInvertedTankDrive()
    // {
    // SpeedController rightSC = new Talon(0);
    // SpeedController leftSC = new Talon(1);
    // Encoder rightEnc = new Encoder(0, 1);
    ...
    // Gyro gyro = new AnalogGyro(0);
    //
    // DataAccessorFactory.getInstance().getEncoderAccessor().connectSpeedController(0,
    // 0);
    // DataAccessorFactory.getInstance().getEncoderAccessor().connectSpeedController(1,
    // 1);
    // DataAccessorFactory.getInstance().getSimulatorDataAccessor().setSpeedControllerModel_Simple(0,
    // new SimpleMotorSimulationConfig(1));
    // DataAccessorFactory.getInstance().getSimulatorDataAccessor().setSpeedControllerModel_Simple(1,
    // new SimpleMotorSimulationConfig(1));
    //
    // DataAccessorFactory.getInstance().getSimulatorDataAccessor().connectTankDriveSimulator(1,
    // 0, 0, 180 / Math.PI);
    // EncoderWrapper rightEncWrapper =
    // SensorActuatorRegistry.get().getEncoders().get(0);
    // EncoderWrapper leftEncWrapper =
    // SensorActuatorRegistry.get().getEncoders().get(1);
    // GyroWrapper gyroWrapper = SensorActuatorRegistry.get().getGyros().get(0);
    // TankDriveGyroSimulator simulator = new
    // TankDriveGyroSimulator(leftEncWrapper, rightEncWrapper, gyroWrapper);
    // simulator.setTurnKp(180 / Math.PI);
    // simulator.setIsReverse(true, true);
    //
    // // Turn Left
    // simulateForTime(90, () ->
    // {
    ...
    // DataAccessorFactory.getInstance().getGyroAccessor().getAngle(0),
    // DOUBLE_EPSILON);
    // Assert.assertEquals(90,
    // DataAccessorFactory.getInstance().getEncoderAccessor().getDistance(0),
    // DOUBLE_EPSILON);
    // Assert.assertEquals(-90,
    // DataAccessorFactory.getInstance().getEncoderAccessor().getDistance(1),
    // DOUBLE_EPSILON);
    // }
