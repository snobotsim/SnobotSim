    ...
    for (int i = 0; i < numDIO; ++i)
    {
        if (HALSIM_GetDIOInitialized(i))
        {
            ...
            else if (dutyCycleMap[i] > 0)
            {
                ...
                if (auto simDevice = HALSIM_GetDutyCycleSimDevice(dutyCycleMap[i] - 1))
                {
                    LabelSimDevice(name, simDevice);
                }
                else
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (ImGui::Combo(name, &val, options, 2))
