...
{
    WindowInfo() = default;
    explicit WindowInfo(const char* name_) :
            name{ name_ }
    {
    }
...
{
    if (initialize)
        gInitializers.emplace_back(std::move(initialize));
}

void HALSimGui::AddExecute(std::function<void()> execute)
{
    if (execute)
        gExecutors.emplace_back(std::move(execute));
}
...
{
    if (menu)
        gMenus.emplace_back(std::move(menu));
}

void HALSimGui::AddOptionMenu(std::function<void()> menu)
{
    if (menu)
        gOptionMenus.emplace_back(std::move(menu));
}
...
void HALSimGui::Main(void*)
{
    ...
    while (!glfwWindowShouldClose(gWindow) && !gExit)
    {
        ...
        // 4 = 1.5x, 5 = 1.75x, 6 = 2x
        int fontScale = std::clamp(gUserScale + static_cast<int>((windowScale - 1.0) * 4), 0,
                kScaledFontLevels - 1);
            ...
            if (ImGui::BeginMenu("View"))
            {
                ...
                if (ImGui::BeginMenu("Zoom"))
                {
                    for (int i = 0; i < kScaledFontLevels && (25 * (i + 2)) <= 200; ++i)
                    {
                        ...
                        bool enabled = (fontScale - gUserScale + i) >= 0 && (fontScale - gUserScale + i) < kScaledFontLevels;
