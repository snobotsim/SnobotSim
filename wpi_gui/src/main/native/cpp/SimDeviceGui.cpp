...
void SimDeviceGui::Hide(const char* name)
{
    gElements[name].visible = false;
}

void SimDeviceGui::Add(std::function<void()> execute)
{
    if (execute)
        gDeviceExecutors.emplace_back(std::move(execute));
}
...
void SimDeviceGui::FinishDevice()
{
    ImGui::PopID();
}
        ...
        HAL_SimDeviceHandle handle)
{
    ...
    if (it != gElements.end() && !it->second.visible)
        return;
...
{
    if (execute)
        SimDeviceGui::Add([=] { execute(param); });
}

void HALSIMGUI_DeviceTreeHide(const char* name)
{
    SimDeviceGui::Hide(name);
}
